...
objects = {
    ...
    2: [images.soil, None, "It's like a desert. Or should that be a dessert?"],
    ...
    18: [images.pipes2, images.pipes2_shadow, "Pipes for the life support system."],
    19: [images.pipes3, images.pipes3_shadow, "Pipes for the life support system."],
    20: [images.door, images.door_shadow, "Safety door. Open automatically for astronauts in functioning spacesuits."],
    ...
    25: [images.door, images.door_shadow, "A locked door. It's opened from Mission Control."],
    ...
    28: [images.rock_large, images.rock_large_shadow, "A rock. Its coarse surface fells like a whetstone.", "the rock"],
    29: [images.rock_small, images.rock_small_shadow, "A small but heavy piece of Martian rock."],
    30: [images.crater, None, "A crater in the planet's surface."],
    31: [images.fence, None, "A fine gauze fence. It helps protect the station from the dust storms."],
    ...
    34: [images.toilet, images.half_shadow, "A sparkling clean bowl."],
    ...
    37: [images.science_lab_table, None, "A table of experiments, analyzing the planet's soil and dust."],
    38: [images.vending_machine, images.full_shadow, "A vending machine. It requires a credit.", "the vending machine"],
    39: [images.floor_pad, None, "A pressure sensor to make sure no one goes out alone."],
    ...
    55: [images.yoyo, None, "A toy made of fine, strong string and plastic.", PLAYER_NAME + "'s yoyo"],
    56: [images.thread, None, "A piece of fine string.", "a piece of string"],
    ...
    67: [images.food_pouch, None, "A dehydrated food pouch. It needs water.", "a dry food pouch"],
    ...
    71: [images.lander, None, "The Poodle, a small space exploration craft. Its black box has a radio sealed inside.", "the Poodle"],
    ...
    75: [images.positioning_system, None, "A working positioning system.", "a positioning system"],
    ...
    78: [images.credit, None, "A small coin for the station's vending machine.", "a station credit"],
...
scenery = {
    26: [[39,8,2]],
    27: [[33,5,5], [33,1,1], [33,1,8],
         [47,5,2], [47,3,10], [47,9,8],
         [42,1,6]],
    ...
    32: [[48,2,2], [48,2,3], [48,2,4],
         [48,3,2], [48,3,3], [48,3,4],
         [48,4,2], [48,4,3], [48,4,4]],
    33: [[13,1,1], [13,1,3], [13,1,8], [13,1,10],
         [48,2,1], [48,2,7], [48,3,6], [48,3,3]],
    34: [[37,2,2], [37,10,4], [32,6,7], [28,5,3]],
    35: [[16,2,9], [16,2,2], [16,3,3], [16,3,8],
         [16,8,9], [16,8,2], [16,1,8], [16,1,3],
         [12,8,6], [12,9,4], [12,9,8],
         [12,7,1], [12,7,11],
         [15,4,6]],
    ...
    41: [[4,3,1], [4,5,1], [4,7,1],
         [10,3,5], [10,5,5], [10,7,5],
         [12,1,1], [12,1,5]],
    ...
    48: [[17,4,1], [17,4,2], [17,4,3], [17,4,4],
         [17,4,5], [17,4,6], [17,4,7],
...
assert check_counter == 161, "Expected 161 scenery items, got " + str(check_counter)
...
def draw():
    ...
    # Lay down floor tiles and items
    ...
    for y in range(room_height):
        for x in range(room_width):
            ...
            # Player cannot walk on 255: it marks spaces used by objects larger than 1 tile
            if item_here not in items_player_may_stand_on + [255]:
                ...
                if objects[item_here][1] is not None: # If there is a shadow
                    ...
                    # Check for horizontal tiling
                    if shadow_image in [images.half_shadow, images.full_shadow]:
                        shadow_width = int(image.get_width() / TILE_SIZE)
                        for tile_number in range(0, shadow_width):
                            draw_shadow(shadow_image, y, x + tile_number)
                    else:
...
# Props are objects that can be picked up and moved between rooms.
# All props must be set up here. Props not yet in the game go in room 0.
# object number: [room, y, x]
props = {
    20: [31,0,4], 21: [26,0,1], 22: [41,0,2], 23: [39,0,5], 24: [45,0,2], 25: [32,0,2], 26: [27,12,5],
    40: [0,8,6],
    53: [45,1,5], 54: [0,0,0], 55: [0,0,0], 56: [0,0,0], 57: [35,4,6], 58: [0,0,0], 59: [31,1,7],
    60: [0,0,0], 61: [36,1,1], 62: [36,1,6], 63: [0,0,0], 64: [27,8,3], 65: [50,1,7], 66: [39,5,6], 67: [46,1,1], 68: [0,0,0], 69: [30,3,3],
    70: [47,1,3], 71: [0, LANDER_Y, LANDER_X], 72: [0,0,0], 73: [27,4,6], 74: [28,1,11], 75: [0,0,0], 76: [41,3,5], 77: [0,0,0], 78: [35,9,11], 79: [26,3,2],
    80: [41,7,5], 81: [29,1,1]
}

checksum = 0
for key, prop in props.items():
    if key != 71: # Skipped because it's location is different each game
        checksum += (prop[0] * key
                     + prop[1] * (key + 1)
                     + prop[2] * (key + 2))
